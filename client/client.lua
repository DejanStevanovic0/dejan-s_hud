...
-- Player status hunger and thirst
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(5000)
		...
		if Config.ui.showHunger == true then
			...
			TriggerEvent('mrpx_status:getStatus', 'hunger', function(status)
				playerStatus['status'][showPlayerStatus] = {
					name = 'hunger',
		...
		if Config.ui.showThirst == true then
			...
			TriggerEvent('mrpx_status:getStatus', 'thirst', function(status)
				playerStatus['status'][showPlayerStatus] = {
					name = 'thirst',
					value = math.floor(100-status.getPercent())
				}
		...
		UpdateFirstJobMoney()
		UpdateSecondJobMoney()

		Citizen.Wait(5000)
	end
end)

-- Vehicle UI when driving test is active
RegisterNetEvent('trew_hud_ui:drivingTest')
AddEventHandler('trew_hud_ui:drivingTest', function(state)
	SendNUIMessage({ action = 'toggleDrivingTest', active = state })
end)
...
AddEventHandler('trew_hud_ui:setInfo', function(info)
	...
	if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.grade_name ~= nil then
		SendNUIMessage({ action = 'setDirectText', id = 'job_name', value = ESX.PlayerData.job.label })
		SendNUIMessage({ action = 'setDirectText', id = 'job_grade', value = ESX.PlayerData.job.grade_label })
	end

	UpdateFirstJobMoney()

	if (ESX.PlayerData.job2 ~= nil and ESX.PlayerData.job2.name ~= nil and ESX.PlayerData.job2.grade ~= nil) then
		ESX.TriggerServerCallback('mrpx:defaultJob2', function(defaultJob)
			if (ESX.PlayerData.job2.name == defaultJob.name and ESX.PlayerData.job2.grade == defaultJob.grade) then
	...
	if ESX.PlayerData.job2 ~= nil and ESX.PlayerData.job2.grade_name ~= nil then
		SendNUIMessage({ action = 'setDirectText', id = 'job2_name', value = ESX.PlayerData.job2.label })
		SendNUIMessage({ action = 'setDirectText', id = 'job2_grade', value = ESX.PlayerData.job2.grade_label })
	end

	UpdateSecondJobMoney()
end)

function UpdateFirstJobMoney()
	if ESX.PlayerData.job == nil or ESX.PlayerData.job.grade_name == nil then
		return
	end

	if (Config.ui.showJob1Money == true) and string.lower(ESX.PlayerData.job.grade_name) == 'boss' then
		SendNUIMessage({ action = 'element', task = 'enable', value = 'society_job1' })
	else
		SendNUIMessage({ action = 'element', task = 'disable', value = 'society_job1' })
		return
	end

	ESX.TriggerServerCallback('trew_hud_ui:getJob1Money', function(money)
		SendNUIMessage({ action = 'setDirectText', id = 'job_name', value = ESX.PlayerData.job.label })
		SendNUIMessage({ action = 'setMoney', id = 'society_job1', value = money })
	end)
end

function UpdateSecondJobMoney()
	if ESX.PlayerData.job2 == nil or ESX.PlayerData.job2.grade_name == nil then
		return
	end

	if (Config.ui.showJob2Money == true) and string.lower(ESX.PlayerData.job2.grade_name) == 'boss' then
		SendNUIMessage({ action = 'element', task = 'enable', value = 'society_job2' })
	else
		SendNUIMessage({ action = 'element', task = 'disable', value = 'society_job2' })
		return
	end

	ESX.TriggerServerCallback('trew_hud_ui:getJob2Money', function(money)
		SendNUIMessage({ action = 'setDirectText', id = 'job2_name', value = ESX.PlayerData.job2.label })
		SendNUIMessage({ action = 'setMoney', id = 'society_job2', value = money })
	end)
end
...
Citizen.CreateThread(function()
    while true do
        ...
        local takeaway = 0.95
			...
			DrawMarker(25,x2,y2,z2 - takeaway, 0, 0, 0, 0, 0, 0, 1.0, 1.0, 10.3, 55, 160, 205, 50, 0, 0, 2, 0, 0, 0, 0)
            else
...
AddEventHandler('mrpx:setJob', function(job)
	...
	if job ~= nil and job.grade_name ~= nil then
		SendNUIMessage({ action = 'setDirectText', id = 'job_name', value = job.label })
		SendNUIMessage({ action = 'setDirectText', id = 'job_grade', value = job.grade_label })

		if (Config.ui.showJob1Money == true) and string.lower(job.grade_name) == 'boss' then
			SendNUIMessage({ action = 'element', task = 'enable', value = 'society_job1' })
		else
			SendNUIMessage({ action = 'element', task = 'disable', value = 'society_job1' })
		end
...
RegisterNetEvent('mrpx:setJob2')
AddEventHandler('mrpx:setJob2', function(job2)
	ESX.PlayerData.job2 = job2

	if (job2 ~= nil and job2.name ~= nil and job2.grade ~= nil) then
		ESX.TriggerServerCallback('mrpx:defaultJob2', function(defaultJob)
			if (job2.name == defaultJob.name and job2.grade == defaultJob.grade) then
				SendNUIMessage({ action = 'element', task = 'disable', value = 'job2' })
			else
				SendNUIMessage({ action = 'element', task = 'enable', value = 'job2' })
			end
	...
	if job2 ~= nil and job2.grade_name ~= nil then
		SendNUIMessage({ action = 'setDirectText', id = 'job2_name', value = job2.label })
		SendNUIMessage({ action = 'setDirectText', id = 'job2_grade', value = job2.grade_label })

		if (Config.ui.showJob2Money == true) and string.lower(job2.grade_name) == 'boss' then
			SendNUIMessage({ action = 'element', task = 'enable', value = 'society_job2' })
		else
			SendNUIMessage({ action = 'element', task = 'disable', value = 'society_job2' })
		end
...
RegisterNetEvent('mlsx:playerLoaded')
AddEventHandler('mlsx:playerLoaded', function(xPlayer)
	local job = ''
	local job2 = ''

	if xPlayer ~= nil and not hasLoaded then
		...
		TriggerEvent('trew_hud_ui:setInfo', info)
	end
end)